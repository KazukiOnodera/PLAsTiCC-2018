...
# -*- coding: utf-8 -*-
"""
Created on Tue Oct 30 20:54:33 2018
...
def aggregate(df, output_path, drop_oid=True):
    ...
    pt = pd.pivot_table(df, index=['object_id'], columns=['year'], 
                        aggfunc=num_aggregations)
    
    pt.columns = pd.Index([f'y{e[2]}_{e[0]}_{e[1]}' for e in pt.columns.tolist()])
    ...
    # year passband
    col = pd.Series([f'{c[2:]}' for c in pt.columns if c.startswith('y0')])
    for c1,c2 in list(combinations(range(3), 2)):
        col1 = (f'y{c1}'+col).tolist()
        col2 = (f'y{c2}'+col).tolist()
