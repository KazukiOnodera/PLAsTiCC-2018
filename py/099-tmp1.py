...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 21 11:37:20 2018
...
def aggregate(df, output_path, drop_oid=True):
    ...
    tr = utils.load_train()
    df = pd.merge(df, tr[['object_id', 'hostgal_photoz']], on='object_id', how='left')
    df['mjd_photoz'] = df['mjd'] / (1 + df.hostgal_photoz)
...
if __name__ == "__main__":
    ...
    df['date'] = df.mjd_photoz.astype(int)
    
    aggregate(df, f'../data/train_{PREF}.pkl')
    
    # test
    if 0:#utils.GENERATE_TEST:
